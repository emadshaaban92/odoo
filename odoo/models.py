...
class AggregateResult:
    ...
    def __init__(
        ...
        def to_key(row):
            return tuple(row[group] for group in self._groupby)

        def to_value(row):
            return {aggregate: row[aggregate] for aggregate in self._aggregates}

        # TODO maybe directly key as (groups, aggregate)
    ...
    def items(self, as_records: bool= False) -> "Iterator[(tuple, tuple)]":
        for groups, aggregations in self._map.items():
            if as_records:
                yield (
                    tuple(self._as_records(self._groupby[i], g) for i, g in enumerate(groups)),
                    tuple(self._as_records(spec, aggregate) for spec, aggregate in aggregations.items())
                )
            else:
    ...
    def keys(self, as_records: bool= False) -> "Iterator[tuple]":
        if not as_records:
            ...
            for groups in self._map.keys():
                yield tuple(self._as_records(self._groupby[i], g) for i, g in enumerate(groups))

    def _flexible_key(self, key) -> 'tuple':
        if len(self._groupby) == 0 and (key is None or key == ()):
            return ()
