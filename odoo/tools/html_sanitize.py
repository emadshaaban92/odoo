...
def html_sanitize(
    ...
    if not cleaned_src or cleaned_src.isspace():
        return cleaned_src

    cleaned_src = cleaned_src.strip()

    if '<p' not in cleaned_src and cleaned_src[0] != '<':
        # ensure that a simple text will be embed into a <p/>
        cleaned_src = f'<p>{cleaned_src}</p>'

    return markupsafe.Markup(cleaned_src)
