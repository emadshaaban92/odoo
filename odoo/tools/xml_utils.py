...
def _fetch_xsd_attachment(env, xsd_name, url=None):
    """ Fetch an XSD attachment from the database.

    An XSD attachment is identified by its `xsd_name` (prefixed if applicable) and its `url` (if provided).
    ...
    :param str url: the URL used to download the XSD, if any
    :return: XSD attachment related to xsd_name and url (empty if none found)
    """
    search_domain = [('name', '=', xsd_name), ('type', '=', 'binary')]
    if url:
        search_domain.append(('url', '=', url))
...
def _check_with_xsd(tree_or_str, stream, env=None, prefix=None, url=None):
    ...
    :param str url: optional url from which the XSD file was downloaded from
        (to be used with `stream` as the attachment name)
    """
    ...
    if env:
        ...
        if isinstance(stream, str) and stream.endswith('.xsd'):
            attachment = _fetch_xsd_attachment(env, stream, url)
            if not attachment:
...
def load_xsd_files_from_url(env, xsd_info, request_max_timeout=10, modify_xsd_content=None):
    ...
    :param dict xsd_info: a dictionary containing the following information:
        ...
        'save_url: boolean, if false, the attachment won't be saved with the url. This can be useful when multiple main
                   xsd files require the same files (imports), and it avoids having multiple copies of the same file.
                   note: it lowers collision avoidance
    ...
    save_url = xsd_info.get('save_url', True)
    ...
    if not is_zip:
        fetched_attachment = _fetch_xsd_attachment(env, prefixed_xsd_name, save_url and url)
        if fetched_attachment:
            return fetched_attachment
    ...
    for file_path in archive.namelist():
        ...
        attachment = _fetch_xsd_attachment(env, prefixed_file_name, save_url and url)
        if attachment:
...
def validate_xml_from_attachment(env, xml_content, xsd_info, reload_files_function=None):
    ...
    try:
        ...
        _check_with_xsd(xml_content, prefixed_xsd_name, env, prefix, url)
