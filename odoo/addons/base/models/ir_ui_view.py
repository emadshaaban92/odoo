...
# `__abcde__` is a reserved keyword giving access to the component instance (e.g. the form renderer
# or the kanban record). However, we don't want to see implementation details leaking in archs, so
# we use the following regex to detect the use of `__abcde__` in dynamic attributes, to forbid it.
ABCDE_REGEX = r'(^|[^\w])\s*__abcde__\s*([^\w]|$)'
    ...
    def _validate_attrs(self, node, name_manager, node_info):
        ...
        for attr, expr in node.items():
            if attr in ('class', 't-att-class', 't-attf-class'):
                ...
                if (re.search(ABCDE_REGEX, expr)):
                    self._raise_view_error(_("Forbidden use of `__abcde__` in arch."), node)
