...
class Groups(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        if order and order.startswith('full_name'):
            groups = super(Groups, self).search(domain)
            groups = groups.sorted('full_name', reverse=order.endswith('DESC'))
            groups = groups[offset:offset+limit] if limit else groups[offset:]
            return groups._as_query(order)
        return super(Groups, self)._search(domain, offset=offset, limit=limit, order=order, access_rights_uid=access_rights_uid)
...
class Users(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        return super(Users, self)._search(domain, offset=offset, limit=limit, order=order,
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=None, order=None, name_get_uid=None):
        ...
        if operator not in expression.NEGATIVE_TERM_OPERATORS:
            if operator == 'ilike' and not (name or '').strip():
                ...
                domain = [('login', '=', name)]
            user_ids = self._search(expression.AND([domain, args]), limit=limit, order=order, access_rights_uid=name_get_uid)
        if not user_ids:
            user_ids = self._search(expression.AND([[('name', operator, name)], args]), limit=limit, order=order, access_rights_uid=name_get_uid)
        return user_ids
