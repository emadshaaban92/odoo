...
class TestRatingPerformance(TestRatingCommon):
    ...
    def _test_rating_last_value_perfs(self, model, partner_field):
        RECORD_COUNT = 100
        ...
        with self.assertQueryCount(employee=1514):  # tmf 1514 / com 1514
            record_ratings = self.env[model].create([{
                partner_field: partners[idx].id,
                'name': 'Test Rating',
        ...
        return record_ratings

    @users('employee')
    @warmup
    def test_rating_last_value_perfs(self):
        self._test_rating_last_value_perfs('mail.test.rating.thread', 'partner_id')

    @users('employee')
    @warmup
    def test_rating_last_value_perfs_with_rating_mixin(self):
        record_ratings = self._test_rating_last_value_perfs('mail.test.rating', 'customer_id')
