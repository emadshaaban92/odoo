...
options.registry.ImageComparison = options.Class.extend({
     ...
     */
    onBuilt() {
        for (const spanEl of this.$target[0].querySelectorAll('.o_description > span, .o_media_right, .o_media_left')) {
    ...
    showZoomButton(previewMode, widgetValue, params) {
        ...
        if (widgetValue && !buttonEl) {
            ...
            newButtonEl.className = 'btn o_enlarge_button me-1 mb-1 o_not_editable';
