...
class Stage(models.Model):
    ...
    def remove_personal_stage(self): # NOTE: could it be merged with the unlink method ? (why separate method to deal with personal stage and other stage ?? # TODO: Import ValidationError
        """
        ...
        if groupby and groupby[0] == "personal_stage_type_ids" and (len(groupby) == 1 or lazy): #To update personal...ids # TODO: populates personal stages if it does not exist for the user
            ...
            if nb_tasks_ws:
                for task in nb_tasks_ws:
                    task.personal_stage_type_ids |= stages[0]
