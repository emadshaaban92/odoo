...
class TestMailComposerMassMailing(TestMailComposer):
    ...
    def test_composer_mass_mailing_auto_delete_message(self):
        """Check that setting auto_delete_message effectively removes the messages."""
    ...
    def test_composer_mass_mailing_no_template_auto_delete(self):
        ...
        self.assertEqual(len(self._new_mails.exists().filtered(lambda mail: not (mail.to_delete or mail.auto_delete))), 2,
                         'Mails should not be marked to_delete when sent from archived mailing')
