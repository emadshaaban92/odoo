...
class TestSaleTimesheetProjectProfitability(TestCommonSaleTimesheet):
    ...
    def test_get_project_profitability_items(self):
        ...
        """
        # we remove the project_non_billable from the project_ids of the analytic account to enable the computation of manually added aal
        self.project_task_rate.analytic_account_id.project_ids = self.project_task_rate
        ...
        # Adding an extra cost/revenue to make sure those are computed correctly.
        ...
        self.assertDictEqual(
            ...
            {
                'revenues': {
                    'data': [{'id': 'other_revenues', 'sequence': 12, 'invoiced': 100.0, 'to_invoice': 0.0, 'action': None}],
                    'total': {'invoiced': 100.0, 'to_invoice': 0.0},
                },
                'costs': {
                    'data': [
                        {'id': 'other_costs', 'sequence': 13, 'billed': -100.0, 'to_bill': 0.0, 'action': None},
                        {
                            'id': 'billable_manual',
