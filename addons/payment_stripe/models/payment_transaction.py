...
class PaymentTransaction(models.Model):
    ...
    def _stripe_create_checkout_session(self):
        ...
        """
        def get_linked_pmts():
            linked_pmts_ = linked_pms
            card_pms = [
                self.env.ref(f'payment.payment_icon_cc_{pm_code_}')
                for pm_code_ in ('visa', 'mastercard', 'american_express', 'discover')
            ]
            if any(pm_.name.lower() in linked_pms for pm_ in card_pms):
                linked_pmts_ += ['card']
            return linked_pmts_
        ...
        pm_filtered_pmts = filter(
            ...
            lambda pmt: pmt.name in get_linked_pmts() or pmt.name not in existing_pms,
