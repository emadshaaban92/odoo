...
class PaymentTransaction(models.Model):
    ...
    def _send_capture_request(self, amount_to_capture=None):
        ...
        if child_capture_tx:
            ...
            # reference associated with the original payment request.
            child_capture_tx.provider_reference = response_content.get('pspReference')
        ...
        if event_code == 'AUTHORISATION':
            ...
            if source_tx:
                ...
                if source_tx.amount == converted_notification_amount:  # Full capture/void
                    ...
                    if tx and tx.amount != converted_notification_amount:
                        # If the void was requested expecting a certain amount, but in between,
            ...
            if not tx:  # The refund was initiated from Adyen
                ...
                if source_tx:
                    ...
                    tx = self._adyen_create_child_tx_from_notification_data(
                        source_tx, notification_data, operation='refund'
                    )
    ...
    def _adyen_create_child_tx_from_notification_data(
            self, source_tx, notification_data, operation=None
    ):
        """ Create a child transaction based on Adyen data.

        :param recordset source_tx: The source transaction for which a child is initiated, as
                                    a `payment.transaction` recordset
        :param dict notification_data: The notification data sent by the provider
        :param string operation: The operation of the child, e.g. 'refund'.
                                 Default to `self.operation`
        ...
        return source_tx._create_child_transaction(
            amount=converted_amount, provider_reference=provider_reference, operation=operation
        )
