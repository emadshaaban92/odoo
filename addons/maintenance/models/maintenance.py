...
class MaintenanceEquipment(models.Model):
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=None, order=None, name_get_uid=None):
        ...
        if name and operator not in expression.NEGATIVE_TERM_OPERATORS and operator != '=':
            equipment_ids = self._search([('name', '=', name)] + args, limit=limit, order=order, access_rights_uid=name_get_uid)
        return equipment_ids or super()._name_search(name, args, operator, limit, order, name_get_uid)
