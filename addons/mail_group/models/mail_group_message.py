...
class MailGroupMessage(models.Model):
    ...
    attachment_ids = fields.Many2many(related='mail_message_id.attachment_ids', inverse=True)
    author_id = fields.Many2one(related='mail_message_id.author_id', inverse=True)
    email_from = fields.Char(related='mail_message_id.email_from', inverse=True)
    email_from_normalized = fields.Char('Normalized From', compute='_compute_email_from_normalized', store=True)
    body = fields.Html(related='mail_message_id.body', inverse=True)
    subject = fields.Char(related='mail_message_id.subject', inverse=True)
    # Thread
