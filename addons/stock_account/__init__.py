...
def _configure_journals(cr, registry):
    ...
    for company in env['res.company'].search([('chart_template', '!=', False)]):
        ...
        if not properties:
            ...
            if not journal:
                journal_data = ChartTemplate._get_account_journal(template_code)
                journal = Journal.create(journal_data['inventory_valuation'])
            Property._set_default('property_stock_journal', 'product.category', journal.id, company)

        # Property Stock Accounts
        template_data = ChartTemplate._get_data(template_code, 'account.account')
        for field in (
