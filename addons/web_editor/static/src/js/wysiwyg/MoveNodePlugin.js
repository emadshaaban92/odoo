...
const widgetMoveSize = 20;

const allowedElements = 'h1, h2, h3, p, hr, pre, blockquote, ul, ol, table, .o_knowledge_behavior_anchor, .o_text_columns';
...
export class MoveNodePlugin {
    ...
    start() {
        ...
        const positionOption = {
            position: 'before',
            targetContainerId: 'oe-avatars-overlap-container'
        };
        ...
        this.observer = new MutationObserver(() => {
            this._removeMoveWidget && this._removeMoveWidget();
            this._updateHooks();
        });
    ...
    _updateHooks() {
        ...
        this._editableRect.x = this._editableRect.x + paddingLeft - (widgetContainerWidth + 5);
        this._editableRect.width = this._editableRect.width - paddingLeft + (widgetContainerWidth + 5);
        ...
        for (const element of elements) {
            ...
            if (element.tagName === 'HR') {
                hookBox = new DOMRect(
                    elementBox.x - containerRect.left - widgetContainerWidth,
                    elementBox.y - containerRect.top - marginTop,
                    elementBox.width + widgetContainerWidth,
    ...
    _getMovableElements() {
        return [...new Set([...this._editable.querySelectorAll(allowedElements)])]
            .filter((node) =>
    ...
    _setMovableElement(movableElement) {
        ...
        if (this._currentMovableElement.tagName.match(/H[1-6]/)) {
            anchorY += (anchorBlockRect.height - widgetMoveSize) / 2;
        }
        ...
        this._moveWidget.style.width = `${widgetMoveSize}px`;
        this._moveWidget.style.height = `${widgetMoveSize}px`;
        this._moveWidget.style.top = `${anchorY - containerRect.y - moveWidgetOffsetTop}px`;
        this._moveWidget.style.left = `${anchorX - containerRect.x - widgetContainerWidth}px`;
        const timeout = setTimeout(()=> {
            return this._moveWidget.classList.add('oe-sidewidget-move--show');
        });

        // Keep a reference for the closure.
        const moveWidget = this._moveWidget;
        this._removeMoveWidget = () => {
            clearTimeout(timeout);
            this._editor.enableAvatars(movableElement);
            moveWidget.classList.remove('oe-sidewidget-move--show');
            const transitionend = () => {
                moveWidget.removeEventListener('transitionend', transitionend);
                moveWidget.remove();
            };
            moveWidget.addEventListener('transitionend', transitionend);

            // Ensure the widget is removed even if the the transition does not end.
            setTimeout(() => moveWidget.remove(), 200);
            this._removeMoveWidget = undefined;
            this._currentMovableElement = undefined;
        }

        if (this._scrollableElement) {
            new SmoothScrollOnDrag(this.wyiswyg, $(this._moveWidget), $(this._scrollableElement), {
                scrollBoundaries: {
                    right: false,
                },
    ...
    _startDropzones(movableElement, containerRect, directions = ['north', 'south']) {
        ...
        for (const element of elements) {
            ...
            const dropzoneRect = new DOMRect(
                originalRect.left - marginLeft - widgetContainerWidth,
                originalRect.top - marginTop,
                originalRect.width + marginLeft + marginRight + widgetContainerWidth,
