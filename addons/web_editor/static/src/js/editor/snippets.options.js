...
const ButtonUserValueWidget = UserValueWidget.extend({
    ...
    start: function (parent, title, options) {
        ...
        if (this.imgEl) {
            ...
            const svgEl = this.$el[0].querySelector('svg');
            const positionHolderEl = document.createElement('span');
            if (svgEl) {
                svgEl.replaceWith(positionHolderEl);
            }
            const text = this.$el[0].textContent.replace(/\u200B+/g, '').trim();
            if (svgEl) {
                positionHolderEl.replaceWith(svgEl);
            }
            if (text) {
...
const BackgroundPositionMixin = {
    ...
    _applyPosition: async function (position) {
        this.$target.css('background-position', position);
    },
    /**
    ...
    _toggleBgOverlay: function (activate) {
        ...
        ['background-attachment', 'background-image', 'background-position', 'background-size', 'background-repeat', 'width', 'height'].forEach(
            cssProperty => this.$bgDragger.css(cssProperty, this.$target.css(cssProperty))
        );
