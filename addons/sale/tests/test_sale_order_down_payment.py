...
class TestSaleOrderDownPayment(TestSaleCommon):
    ...
    def test_tax_fixed_amount_breakdown(self):
        ...
        expected = [
            ...
            ['account_id',               'tax_ids',              'balance',      'price_total'],
            # base lines
            [self.revenue_account.id,    self.env['account.tax'], -down_pay_amt, down_pay_amt ],
            # receivable
            [self.receivable_account.id, self.env['account.tax'], down_pay_amt,  0            ],
        ]
        self._assert_invoice_lines_values(invoice.line_ids, expected)
    ...
    def test_analytic_distribution(self):
        ...
        an_acc_01 = self.env['account.analytic.account'].create({'name': 'Account 01', 'plan_id': analytic_plan.id})
        an_acc_02 = self.env['account.analytic.account'].create({'name': 'Account 02', 'plan_id': analytic_plan.id})
        self.sale_order.order_line[0].tax_id = self.tax_15 + self.tax_10
        self.sale_order.order_line[0].analytic_distribution = {an_acc_01.id: 100}
        self.sale_order.order_line[1].tax_id = self.tax_10
        self.sale_order.order_line[1].analytic_distribution = {an_acc_01.id: 50, an_acc_02.id: 50}
        self.sale_order.order_line[2].tax_id = self.tax_10
        self.sale_order.order_line[2].analytic_distribution = {an_acc_01.id: 100}
        self.make_downpayment()
        invoice = self.sale_order.invoice_ids
        down_pay_amt = self.sale_order.amount_total / 2
        # pylint: disable=C0326
        expected = [
            ...
            # base lines
            [self.revenue_account.id,    (self.tax_15 + self.tax_10).ids, -100,         125,           {str(an_acc_01.id): 100}                      ],
            [self.revenue_account.id,    self.tax_10.ids,                 -200,         220,           {str(an_acc_01.id): 75, str(an_acc_02.id): 25}],
