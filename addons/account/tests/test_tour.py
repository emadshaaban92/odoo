...
class TestUi(odoo.tests.HttpCase):
    ...
    def test_01_account_tour(self):
        if self.env.company.country_id != self.env.company.account_fiscal_country_id:
            ...
            company = self.env.user.company_ids.filtered(lambda c: c.country_id.id == country_id.id and c.account_fiscal_country_id.id == country_id.id)
            if company:
        ...
        self.start_tour("/web", 'account_tour', login="admin", timeout=120)
