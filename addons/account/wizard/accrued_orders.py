...
class AccruedExpenseRevenue(models.TransientModel):
    ...
    def _compute_move_vals(self):
        def _get_aml_vals(order, balance, amount_currency, account_id, label="", analytic_distribution=False):
            if not is_purchase:
                balance *= -1
                amount_currency *= -1
            values = {
        ...
        for order in orders:
            if len(orders) == 1 and self.amount:
                ...
                values = _get_aml_vals(order, self.amount, 0, account.id, label=_('Manual entry'), analytic_distribution=order_line.analytic_distribution)
                ...
                for order_line in lines:
                    ...
                    values = _get_aml_vals(order, amount, amount_currency, account.id, label=label, analytic_distribution=order_line.analytic_distribution)
                    move_lines.append(Command.create(values))
        ...
        if not self.company_id.currency_id.is_zero(total_balance):
            ...
            for distribution in orders.order_line.mapped('analytic_distribution'):
                if distribution:
                    for k, v in distribution.items():
                        analytic_distribution.update({k : analytic_distribution.get(k, 0) + distribution[k]})
            for k, v in analytic_distribution.items():
                analytic_distribution[k] = v/len(orders)
