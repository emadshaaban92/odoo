...
class account_journal(models.Model):
    ...
    def mark_bank_setup_as_done_action(self):
        ...
        self.env['onboarding.onboarding.step'].action_save_account_invoice_onboarding_bank_account_step()

    def unmark_bank_setup_as_done_action(self):
        """ Marks the 'bank setup' step as not done in the setup bar and in the company."""
        step_progress = self.env.ref('account.account_invoice_onboarding_bank_account_step', raise_if_not_found=False).current_progress_step_id
        if step_progress:
            step_progress.step_state = 'not_done'
