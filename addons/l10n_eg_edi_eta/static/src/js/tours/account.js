...
if (account_tour.extra && (account_tour.extra.company_account_fiscal_country_code || "") === 'EG') {

    let company_name = account_tour.extra.company_name;

    let post_company_step_idx = _.findIndex(account_tour.steps, step => step.trigger === 'button[name=action_save_onboarding_company_step]');
    // Configure ETA settings on the Customer Invoices journal
    account_tour.steps.splice(post_company_step_idx + 1, 0, {
        ...
        trigger: ".ui-menu-item > a:contains('" + company_name + "').ui-state-active",
        auto: true,
        in_modal: false,
    }, {
    ...
    account_tour.steps.splice(add_line_step_idx + 1, 0, {
