...
class HashMixin(models.AbstractModel):
    ...
    10. The _create_hash_string method will compute the hash of the current record using:
        ...
        - the fields defined in _get_inalterable_hash_fields
    11. If the inalterable_hash, the secure_sequence_number or any of the fields defined in _get_inalterable_hash_fields is modified, an error will be raised
    """
    _name = 'hash.mixin'
    ...
    def _get_inalterable_hash_fields(self):
        ...
        """
        raise NotImplementedError("'_get_inalterable_hash_fields' must be overriden by the inheriting class")
    ...
    def _create_hash_string(self, previous_hash=None):
        ...
        :returns: the hash computed using the previous hash and the fields defined in _get_inalterable_hash_fields
        :rtype: str
        """
        self.ensure_one()
    ...
    def _get_dict_fields_values_to_hash(self, lines=()):
        ...
        self.ensure_one()
        result = {}
        for field in self._get_inalterable_hash_fields():
            result[field] = self._get_field_as_string(self, field)

        for line in lines:
            for field in line._get_inalterable_hash_fields():
    ...
    def write(self, vals):
        for record in self:
            if record.inalterable_hash:
                inalterable_fields = set(record._get_inalterable_hash_fields()).union({'inalterable_hash', 'secure_sequence_number'})
...
class SubHashMixin(models.AbstractModel):
    ...
    This allows us to verify that, if the parent model is hashed, then the fields returned by _get_inalterable_hash_fields()
    cannot be modified.
    """
    ...
    def write(self, vals):
        for record in self:
            violated_fields = set(vals).intersection(record._get_inalterable_hash_fields())
