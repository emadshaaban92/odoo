...
export function useDynamicPlaceholder(ownerField, elementRef) {
    const TRIGGER_KEY = '#';
    ...
    let onDynamicPlaceholderValidate = function(chain, defaultValue) {
        ...
        if (chain) {
            ...
            const newValue = splitedValue[0].replace(triggerKeyReplaceRegex, '') + dynamicPlaceholder + splitedValue[1];
    ...
    let onDynamicPlaceholderClose = function() {
        elementRef?.el.focus();
    }
    ...
    async function open(options) {
        if (!baseModel) {
            return notification.add(
                ownerField.env._t("You need to select a baseModel before opening the dynamic placeholder selector."),
                {type: "danger"}
            );
        }
