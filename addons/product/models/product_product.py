...
class ProductProduct(models.Model):
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        return super(ProductProduct, self)._search(domain, offset=offset, limit=limit, order=order, access_rights_uid=access_rights_uid)
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=None, order=None, name_get_uid=None):
        ...
        if name:
            ...
            if operator in positive_operators:
                product_ids = list(self._search([('default_code', '=', name)] + args, limit=limit, order=order, access_rights_uid=name_get_uid))
                if not product_ids:
                    product_ids = list(self._search([('barcode', '=', name)] + args, limit=limit, order=order, access_rights_uid=name_get_uid))
            if not product_ids and operator not in expression.NEGATIVE_TERM_OPERATORS:
                # Do not merge the 2 next lines into one single search, SQL search performance would be abysmal
                # on a database with thousands of matching products, due to the huge merge+unique needed for the
                # OR operator (and given the fact that the 'name' lookup results come from the ir.translation table
                # Performing a quick memory merge of ids in Python will give much better performance
                product_ids = list(self._search(args + [('default_code', operator, name)], limit=limit, order=order))
                if not limit or len(product_ids) < limit:
                    # we may underrun the limit because of dupes in the results, that's fine
                    limit2 = (limit - len(product_ids)) if limit else False
                    product2_ids = self._search(args + [('name', operator, name), ('id', 'not in', product_ids)], limit=limit2, order=order, access_rights_uid=name_get_uid)
                ...
                product_ids = list(self._search(domain, limit=limit, order=order, access_rights_uid=name_get_uid))
            if not product_ids and operator in positive_operators:
                ptrn = re.compile('(\[(.*?)\])')
                res = ptrn.search(name)
                if res:
                    product_ids = list(self._search([('default_code', '=', res.group(2))] + args, limit=limit, order=order, access_rights_uid=name_get_uid))
            ...
            product_ids = self._search(args, limit=limit, order=order, access_rights_uid=name_get_uid)
        return product_ids
