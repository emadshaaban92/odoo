...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        for order in self:
            order.coupon_point_ids = order.coupon_point_ids.filtered(lambda c: order.pricelist_id in c.coupon_id.program_id.pricelist_ids or len(c.coupon_id.program_id.pricelist_ids) == 0)
    ...
    def _update_programs_and_rewards(self):
        ...
        self.coupon_point_ids = self.coupon_point_ids.filtered(lambda c: self.pricelist_id in c.coupon_id.program_id.pricelist_ids or len(c.coupon_id.program_id.pricelist_ids) == 0)
    ...
    def __try_apply_program(self, program, coupon, status):
        ...
        if(self.pricelist_id in program.pricelist_ids or len(program.pricelist_ids) == 0):
            if coupon:
