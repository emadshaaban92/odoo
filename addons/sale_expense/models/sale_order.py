...
class SaleOrder(models.Model):
    ...
    def _name_search(self, name='', args=None, operator='ilike', limit=None, order=None, name_get_uid=None):
        ...
        if self._context.get('sale_expense_all_order') and self.user_has_groups('sales_team.group_sale_salesman') and not self.user_has_groups('sales_team.group_sale_salesman_all_leads'):
            ...
            return super(SaleOrder, self.sudo())._name_search(name=name, args=domain, operator=operator, limit=limit, order=order, name_get_uid=SUPERUSER_ID)
        return super(SaleOrder, self)._name_search(name=name, args=args, operator=operator, limit=limit, order=order, name_get_uid=name_get_uid)
