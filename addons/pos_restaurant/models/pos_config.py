...
class PosConfig(models.Model):
    ...
    floor_ids = fields.Many2many('restaurant.floor', 'restaurant_floor_pos_config_rel', 'pos_config_id', 'restaurant_floor_id', string='Restaurant Floors', help='The restaurant floors served by this point of sale.')
    ...
    def get_tables_order_count(self):
        """         """
        self.ensure_one()
        tables = self.env['restaurant.table'].search([]).filtered(lambda r: self.ids[0] in r.floor_id.pos_config_ids.ids)
