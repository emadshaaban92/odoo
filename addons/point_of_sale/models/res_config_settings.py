...
class ResConfigSettings(models.TransientModel):
    ...
    pos_share_orders = fields.Boolean(related='pos_config_id.share_orders', readonly=False)
        ...
        for config in self:
            ...
            for old in config.pos_config_id.trusted_config_ids:
                if config.pos_config_id.id not in old.trusted_config_ids.ids:
                    old.add_trusted_config_id(config.pos_config_id.id)
                if old.id in removed_trusted_configs:
                    old.remove_trusted_config_id(config.pos_config_id.id)
