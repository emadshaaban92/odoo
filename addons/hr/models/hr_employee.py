...
class HrEmployeePrivate(models.Model):
    ...
    def search(self, domain, field_names=(), offset=0, limit=None, order=None):
        if self.check_access_rights('read', raise_exception=False):
            return super().search(domain, field_names, offset, limit, order)
        ...
        public = self.env['hr.employee.public'].search(domain, field_names, offset, limit, order)
        ...
        for fname in field_names:
            ...
            self.env.cache.update_raw(employees, self._fields[fname], values)
        return employees
    ...
    def _search(self, domain, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        """
        if self.check_access_rights('read', raise_exception=False):
            return super(HrEmployeePrivate, self)._search(domain, offset=offset, limit=limit, order=order, access_rights_uid=access_rights_uid)
        ids = self.env['hr.employee.public']._search(domain, access_rights_uid=access_rights_uid)
        # the result is expected from this table, so we should link tables
        return super(HrEmployeePrivate, self.sudo())._search([('id', 'in', ids)], offset=offset, limit=limit, order=order)
