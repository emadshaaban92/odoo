...
class ResPartner(models.Model):
    ...
    slide_channel_ids = fields.Many2many(
        ...
        compute='_compute_slide_channel_ids',
    ...
    def _compute_slide_channel_ids(self):
        slide_channel_partners = self.env['slide.channel.partner'].search([
            ('partner_id', 'in', self.ids),
            ('member_status', '!=', 'invited')
        ])
        for partner in self:
            partner.slide_channel_ids = slide_channel_partners.filtered(lambda scp: scp.partner_id == partner).mapped('channel_id')

    def _search_slide_channel_ids(self, operator, value):
        cp_enrolled = self.env['slide.channel.partner'].sudo().search([
    ...
    def _compute_slide_channel_count(self):
        read_group_res = self.env['slide.channel.partner'].sudo()._read_group(
            [('partner_id', 'in', self.ids), ('member_status', '!=', 'invited')],
    ...
    def action_view_courses(self):
        ...
        action['display_name'] = _('Followed Courses')
