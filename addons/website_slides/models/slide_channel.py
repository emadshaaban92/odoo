...
class Channel(models.Model):
    ...
    def _remove_membership(self, partner_ids):
        ...
        self._update_users_karma(partner_ids, joining=False)
    ...
    def _update_users_karma(self, partner_ids, joining=True):
        """ Update the users karma while leaving / joining the course.

            :param bool joining: If true, karma will added for the user(s),
              otherwise (while leaving the course), it will be deducted.
        """
        earned_karma = self._get_earned_karma(partner_ids)
        users = self.env['res.users'].sudo().search([
            ('partner_id', 'in', list(earned_karma)),
        ])
        factor = 1 if joining else -1
        for user in users:
            if earned_karma[user.partner_id.id]:
                user.add_karma(factor * earned_karma[user.partner_id.id])
