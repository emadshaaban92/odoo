...
class ProductProduct(models.Model):
    ...
    def _compute_mrp_product_qty(self):
        ...
        aggregate_res = self.env['mrp.production']._aggregate(domain, ['product_uom_qty:sum'], ['product_id'])
        for product in self:
            if not product.id:
                product.mrp_product_qty = 0.0
                continue
            product.mrp_product_qty = float_round(aggregate_res.get_agg(product.id, 'product_uom_qty:sum', 0), precision_rounding=product.uom_id.rounding)
