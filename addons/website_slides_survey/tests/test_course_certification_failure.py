...
class TestCourseCertificationFailureFlow(TestSurveyCommon):
    def test_course_certification_failure_flow(self):
        ...
        self.assertFalse(channel_partner.active, 'Portal user should have been archived from the course attendee because he failed his last attempt')
        ...
        self.slide_certification.with_user(self.user_portal)._generate_certification_url()
        self.assertTrue(channel_partner.active, 'Portal user should be a unarchived upon joining the course once again')
        ...
        # forces recompute of partner_ids as we delete directly in relation
        self.channel.invalidate_model()
        self.assertTrue(channel_partner.active, 'Portal user should be active')
