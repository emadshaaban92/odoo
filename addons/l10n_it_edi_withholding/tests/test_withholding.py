...
class TestWithholdingAndPensionFundTaxes(TestItEdi):
    ...
    def setUpClass(cls):
        ...
        cls.edi_enasarco_tax_xml = cls._get_withholding_test_file_content('IT00470550013_enasarco.xml')
    ...
    def test_enasarco_tax_import(self):
        ...
        invoice = self.edi_format._create_invoice_from_xml_tree("IT00470550013_enasarco.xml", imported_etree, self.purchase_journal)
        invoice_data = self.get_real_client_invoice_data()
        for line in invoice.line_ids.filtered(lambda x: x.name in [data[0] for data in invoice_data.lines]):
