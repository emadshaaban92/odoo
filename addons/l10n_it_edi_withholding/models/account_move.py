...
class AccountMove(models.Model):
    ...
    def _compute_amount_extended(self):
        for move in self:
            ...
            for line in [line for line in move.line_ids if move.is_invoice(True) and line.tax_line_id]:
    ...
    def _l10n_it_edi_prepare_fatturapa_tax_details(self, tax_details, reverse_charge_refund=False):
        tax_details['tax_details'] = {k: v for k, v in tax_details['tax_details'].items() if v['tax']._l10n_it_filter_kind('vat')}
    ...
    def _l10n_it_edi_filter_fatturapa_tax_details(self, line, tax_values):
        ...
        return tax_values['tax_repartition_line'].factor_percent >= 0 and line['taxes']._l10n_it_filter_kind('vat').amount >= 0
    ...
    def _prepare_fatturapa_export_values(self):
        ...
        if enasarco_taxes:
            enasarco_values = {}
            all_tax_details = self._prepare_edi_tax_details(filter_to_apply=lambda line, tax_values: self.env['account.tax'].browse([tax_values['id']]).l10n_it_pension_fund_type == 'TC07')
            for detail in all_tax_details['tax_details_per_record'].values():
