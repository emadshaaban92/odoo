...
class ResourceMixin(models.AbstractModel):
    ...
    company_id = fields.Many2one(
        ...
        index=True, related='resource_id.company_id', store=True, inverse=True)
    resource_calendar_id = fields.Many2one(
        'resource.calendar', 'Working Hours',
        default=lambda self: self.env.company.resource_calendar_id,
        index=True, related='resource_id.calendar_id', store=True, inverse=True)
    tz = fields.Selection(
        string='Timezone', related='resource_id.tz', inverse=True,
