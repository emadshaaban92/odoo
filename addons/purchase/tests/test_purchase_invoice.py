...
class TestPurchaseToInvoiceCommon(AccountTestInvoicingCommon):
    ...
    @classmethod
    def init_purchase(cls, partner=None, confirm=False, products=None, amounts=None, taxes=None, company=False):
        ...
        for amount in (amounts or []):
            with po_form.order_line.new() as line_form:
                ...
                line_form.product_qty = 1
                line_form.product_uom = cls.product_order.uom_id
...
class TestInvoicePurchaseMatch(TestPurchaseToInvoiceCommon):
    ...
    def test_total_match_via_partner(self):
        po = self.init_purchase(confirm=True, partner=self.partner_a, amounts=[100])
        invoice = self.init_invoice('in_invoice', partner=self.partner_a, amounts=[100])
    ...
    def test_total_match_via_po_reference(self):
        po = self.init_purchase(confirm=True, amounts=[100])
        invoice = self.init_invoice('in_invoice', partner=self.partner_a, amounts=[100])
    ...
    def test_subset_total_match_prefer_purchase(self):
        po = self.init_purchase(confirm=True, amounts=[100, 200])
        invoice = self.init_invoice('in_invoice', partner=self.partner_a, amounts=[100])

        invoice._find_and_set_purchase_orders(
            ['my_match_reference'], invoice.partner_id.id, invoice.amount_total, prefer_purchase_line=True)
        additional_unmatch_po_line = po.order_line.filtered(lambda _line: _line.price_unit == 200)

        self.assertTrue(invoice.id in po.invoice_ids.ids)
        self.assertTrue(invoice.line_ids.filtered(
            lambda _line: _line.purchase_line_id == additional_unmatch_po_line).quantity == 0)
    ...
    def test_po_match_prefer_purchase(self):
        po = self.init_purchase(confirm=True, amounts=[100, 200])
        invoice = self.init_invoice('in_invoice', amounts=[999])
    ...
    def test_no_match(self):
        po = self.init_purchase(confirm=True, partner=self.partner_a, amounts=[100])
        invoice = self.init_invoice('in_invoice', partner=self.partner_b, amounts=[100])
