 ...
 * @property {Deferred<string|undefined>} deferred
 *
 ...
 *
 * @typedef {[number, string]} NameGetRPCResult
 */
...
export class DisplayNameRepository {
     ...
     * @param {string} model
     * @param {number} id
     * @returns {Deferred<string|undefined>}
     */
    async waitForDisplayName(model, id) {
    ...
    setDisplayName(model, id, displayName) {
        ...
        this._displayNames[model][id] = {
            state: "COMPLETED",
            deferred: Promise.resolve(displayName),
            value: displayName,
        };
    }
     ...
     * it will return undefined and fetch the display name in the background.
     *
     * @param {string} model
     * @param {number} id
     ...
     * @param {NameGetRPCResult|undefined} result
     *
     * @private
     */
