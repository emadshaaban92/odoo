...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges({ shouldInline = true } = {}) {
        ...
        const initialEditableCloned = $editable[0].cloneNode(true);
        ...
        const iframeHTMLNode = $editable[0].closest('html').cloneNode(true);
        const clonedBody = iframeHTMLNode.querySelector('body');
        const clonedIframeTarget = iframeHTMLNode.querySelector('#iframe_target');
        ...
        const editableClone = iframeHTMLNode.querySelector('.note-editable');
        const iframePromise = new Promise((resolve) => {
            iframe.addEventListener("load", resolve);
        });
        document.body.append(iframe);
        iframe.contentDocument.firstChild.replaceWith(iframeHTMLNode);
        await iframePromise;
        ...
        this.wysiwyg.odooEditor.editable.replaceChildren(...[...initialEditableCloned.children]);
