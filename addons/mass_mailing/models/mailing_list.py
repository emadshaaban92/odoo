...
class MassMailingList(models.Model):
    ...
    def _update_subscription_from_email(self, email, opt_out=True, force_message=None):
        ...
        if opt_out:
            current_opt_in = contacts.subscription_list_ids.filtered(lambda sub: not sub.opt_out and sub.list_id in self)
            if current_opt_in:
        ...
        for contact in contacts:
            ...
            if force_message:
                ...
                body = '<p>%s</p><ul>%s</ul>' % (
                    ...
                    ''.join('<li>%s</li>' % name for name in updated.mapped('name')),
                )
            else:
                body = '<p>%s</p><ul>%s</ul>' % (
