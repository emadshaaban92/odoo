...
'default_template_id' : %d}""" % (template.model, template.id),
    ...
    def _generate_template_recipients(self, res_ids, render_fields,
        ...
        # render fields dynamically which generates recipients
            ...
            fields_torender = [
                ...
                if field in ['email_cc', 'email_to', 'partner_to']
            ]
    ...
    def _generate_template(self, res_ids, render_fields,
        ...
        for _lang, (template, template_res_ids) in self._classify_per_lang(res_ids).items():
            ...
            fields_torender = [
                field for field in render_fields
                if field not in ['attachment_ids',  # attachments
                                 'email_cc',  # recipients
                                 'email_to',  # recipients
            ...
            # render recipients
            if set(render_fields) & {'email_cc', 'email_to', 'partner_to'}:
                template._generate_template_recipients(
                    template_res_ids, render_fields,
                    render_results=render_results,
                    partners_only=partners_only
                )

            # render scheduled_date
            if 'scheduled_date' in render_fields:
            ...
            if set(render_fields) & {'attachment_ids', 'report_template_ids'}:
