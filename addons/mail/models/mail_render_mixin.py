...
class MailRenderMixin(models.AbstractModel):
    ...
    @api.model
    def _render_template_qweb_view(self, view_or_xmlid, model, res_ids,
                                   add_context=None, options=None):
        ...
        :param str/int/record view_xmlid: source QWeb template. It should be an
        ...
        view_ref = view_or_xmlid.id if isinstance(view_or_xmlid, models.BaseModel) else view_or_xmlid
        for record in self.env[model].browse(res_ids):
            variables['object'] = record
            try:
                render_result = self.env['ir.qweb']._render(
