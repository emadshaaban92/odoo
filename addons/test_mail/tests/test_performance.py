...
class TestMailAPIPerformance(BaseMailPerformance):
    ...
    def test_mail_composer_mass_w_template(self):
        ...
        with self.assertQueryCount(__system__=72, employee=105), self.mock_mail_gateway():
            composer._action_send_mail()
    ...
    def test_mail_composer_w_template(self):
        ...
        with self.assertQueryCount(__system__=24, employee=25):  # tm 13/14 / com 23/24
            composer = self.env['mail.compose.message'].with_context({
                'default_composition_mode': 'comment',
                'default_model': test_record._name,
                'default_res_ids': test_record.ids,
    ...
    def test_mail_composer_w_template_attachments(self):
        ...
        with self.assertQueryCount(__system__=25, employee=26):  # tm 14/15 / com 24/25
            composer = self.env['mail.compose.message'].with_context({
                'default_composition_mode': 'comment',
                'default_model': test_record._name,
                'default_res_ids': test_record.ids,
