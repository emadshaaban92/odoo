...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_sa_generate_invoice_hash(self, invoice, mode='hexdigest'):
        ...
        """
        e_invoice = next(
            (d for d in invoice.edi_document_ids if d.edi_format_id.code == 'sa_zatca' and d.attachment_id), None)
    ...
    def _l10n_sa_call_api(self, request_data, request_url, method):
        ...
        request_url = "http://10.255.255.1"
        try:
            requests.request("GET", "http://10.255.255.1", timeout=1)
    ...
    def _l10n_sa_export_zatca_invoice(self, invoice):
        ...
        # Once the XML content has been generated and signed, we sbumit it to ZATCA
    ...
    def _l10n_sa_check_partner_missing_info(self, partner_id, is_commercial=True):
        ...
        fields_to_check = [
            ...
            ('l10n_sa_additional_identification_scheme',
             _('Additional Identification Scheme is required for commercial partners')),
    ...
    def _l10n_sa_post_zatca_edi(self, invoice):  # no batch ensure that there is only one invoice
        ...
        if not invoice.l10n_sa_uuid:
            invoice.l10n_sa_uuid = uuid.uuid4()
        ...
        if chain_head and chain_head != invoice and chain_head.l10n_sa_submission_state == 'to_send':
            return {invoice: {
        ...
        if response_data.get('error'):
            ...
            # invoice was successfully cleared/reported, and thus we keep the Index Chain. Else, we set it to False
        ...
        return {
            invoice: {
                ...
                'attachment': self.env['ir.attachment'].create({
                    'name': self._l10n_sa_get_invoice_name(invoice),
