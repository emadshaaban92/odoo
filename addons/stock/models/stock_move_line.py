...
class StockMoveLine(models.Model):
    ...
    @api.depends('product_packaging_id', 'product_uom_id', 'reserved_qty')
    ...
    @api.depends('product_packaging_id', 'product_uom_id', 'qty_done')
    def _compute_product_packaging_qty_done(self):
        ...
        for line in self:
            if not line.product_packaging_id:
                ...
                quantity_done = line.product_id.uom_id._compute_quantity(line.qty_done, line.product_id.uom_id)
                line.product_packaging_qty_done = float_round(
                    quantity_done / line.product_packaging_id.qty,
    ...
    def compute_product_packaging_qty(self, qty):
        ...
        return self.move_id.compute_product_packaging_qty(qty)
    ...
    def _get_aggregated_product_quantities(self, **kwargs):
        ...
        def get_aggregated_properties(move_line=False, move=False):
            ...
            return (line_key, name, description, uom, move)

        def compute_packaging(aggregated_move_lines):
            # Compute the packaging quantities for the lines
            for line_key in aggregated_move_lines:
                line = aggregated_move_lines[line_key]
                if line['move'] and line['move'].product_packaging_id:
                    line['packaging_name'] = line['move'].product_packaging_id.name
                    line['packaging_qty'] = line['move'].compute_product_packaging_qty(line['qty_ordered'])
                    line['packaging_qty_done'] = line['move'].compute_product_packaging_qty(line['qty_done'])
        ...
        for move_line in self:
            if kwargs.get('except_package') and move_line.result_package_id:
                continue
            line_key, name, description, uom, move = get_aggregated_properties(move_line=move_line)
