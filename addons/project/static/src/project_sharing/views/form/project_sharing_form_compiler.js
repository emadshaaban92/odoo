...
export class ProjectSharingChatterCompiler extends ViewCompiler {
    ...
    compile(node, params) {
        ...
        if (chatterContainerHookXml) {
            setAttributes(chatterContainerHookXml, {
                "t-if": `__abcde__.uiService.size >= ${SIZES.XXL}`,
            });
    ...
    compileChatter(node) {
        return compileChatter(node, {
            resId: '__abcde__.model.root.resId or undefined',
            resModel: '__abcde__.model.root.resModel',
            projectSharingId: '__abcde__.model.root.context.active_id',
        });
    }
}
...
registry.category("form_compilers").add("portal_chatter_compiler", {
    ...
    fn: (node) =>
        compileChatter(node, {
            resId: "__abcde__.props.record.resId or undefined",
            resModel: "__abcde__.props.record.resModel",
            projectSharingId: "__abcde__.props.record.context.active_id",
        }),
});
