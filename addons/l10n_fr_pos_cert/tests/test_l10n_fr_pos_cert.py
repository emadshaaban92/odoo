...
class TestL10nFrPosCertInalterableHash(TestPoSCommon):
    ...
    def test_l10n_fr_pos_order_get_dict_fields_values_to_hash(self):
        """
        Knowing that:
        pos.order()._get_fields_used_by_hash() = ['date_order', 'user_id', 'lines', 'payment_ids', 'pricelist_id', 'partner_id',
                                                  'session_id', 'pos_reference', 'sale_journal', 'fiscal_position_id']
        pos.order.line()._get_fields_used_by_hash() = ['notice', 'product_id', 'qty', 'price_unit', 'discount',
        ...
        dict_to_hash = order._get_dict_fields_values_to_hash()
        line = order.lines[0]
    ...
    def test_l10n_fr_pos_cert_hash_integrity_report(self):
        ...
        # No record yet
        pos_orders_check = self.env['report.l10n_fr_pos_cert.report_pos_hash_integrity']._check_hash_integrity(self.company_fr._is_accounting_unalterable(), orders, 'date_order')
        self.assertEqual(pos_orders_check['status'], 'no_record')
        self.assertEqual(pos_orders_check['msg'], _("There isn't any record flagged for data inalterability."))

        # Everything should be correctly hashed and verified
        orders |= (
            self._create_pos_order("order2", 100)
            | self._create_pos_order("order3", 200)
            | self._create_pos_order("order4", 300)
            | self._create_pos_order("order5", 400)
            | self._create_pos_order("order6", 500)
        )
        orders.state = 'paid'
        pos_orders_check = self.env['report.l10n_fr_pos_cert.report_pos_hash_integrity']._check_hash_integrity(self.company_fr._is_accounting_unalterable(), orders, 'date_order')
        ...
        self.assertEqual(pos_orders_check['msg'], _('Corrupted data on record %s with id %s', orders[2].name, orders[2].id))

        # Let's try with the inalterable_hash field itself
        Model.write(orders[2], {'date_order': date_order})
        Model.write(orders[-1], {'inalterable_hash': 'fake_hash'})
        pos_orders_check = self.env['report.l10n_fr_pos_cert.report_pos_hash_integrity']._check_hash_integrity(self.company_fr._is_accounting_unalterable(), orders, 'date_order')
        self.assertEqual(pos_orders_check['status'], 'corrupted')
        self.assertEqual(pos_orders_check['msg'], _('Corrupted data on record %s with id %s', orders[-1].name, orders[-1].id))
