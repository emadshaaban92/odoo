...
def get_xml_records():
    ...
    for filename in Path.cwd().glob(f'addons/*/data/*.xml'):
        module = str(filename).split('/')[-3]
        try:
    ...
    for filename in Path.cwd().glob(f'addons/*/demo/*.xml'):
        try:
...
def split_template_from_company(all_records, module):
    ...
    for _record_id, record in chart_templates.items():
        ...
        record['children'].pop('spoken_languages', None)
    ...
    if company_record.get('children'):
        company_record['id'] = Unquoted("self.env.company.get_external_id()[self.env.company.id]")
    ...
    for (module, template), records in all_records.items():
        for model in ['account.fiscal.position.tax', 'account.fiscal.position.tax.template']:
            for record in records.pop(model, {}).values():
                ...
                all_fpos[_id]['children'].setdefault('tax_ids', [])
                all_fpos[_id]['children']['tax_ids'].append((0, 0, record))
        for model in ['account.fiscal.position.account', 'account.fiscal.position.account.template']:
            for record in records.pop(model, {}).values():
                _id = ref_module(str(record['children'].pop('position_id')._original_value), module)
                all_fpos[_id]['children'].setdefault('account_ids', [])
                all_fpos[_id]['children']['account_ids'].append((0, 0, record))
    ...
    for (module, template), records in all_records.items():
        for model in ['account.reconcile.model.line', 'account.reconcile.model.line.template']:
            for record in records.pop(model, {}).values():
                ...
                all_models[_id]['children'].setdefault('line_ids', [])
                all_models[_id]['children']['line_ids'].append((0, 0, record))



def load_translations(module):
    paths = Path.cwd().glob(f'addons/{module}/i18n*/*.po*')
...
def do_translate():
    ...
    for (module, old_template), records in all_records.items():
        ...
        if parent := records['account.chart.template'][old_template]['children'].get('parent_id'):
            parent_template = MAPPING.get(ref_module(str(parent._value), module))
        else:
            parent_template = ''
        translations = load_translations(module)

        for model in ['account.account', 'account.group', 'account.tax.group']:
            if model in records:
        ...
        for model in ['account.account', 'account.group', 'account.tax.group']:
            ...
            if template != 'generic_coa':
                model += f"-{template}"
            if content:
                save_new_file(f"addons/{module}/data/template/", f"{model}.csv", content)
        ...
        mapping = {
            ...
            "account.tax":                      f"_get_{template}_account_tax",
            "account.fiscal.position":          f"_get_{template}_fiscal_position",
        ...
        if extra_functions:
            content += (
                f"    def _get_{template}_chart_template_data(self, template_code):\n"
                "        return {\n"
            )
            for model, function_name in extra_functions:
                content += f"            '{model}': self.{function_name}(template_code),\n"
            content += "        }\n\n"
        ...
        save_new_file(f"addons/{module}/models/", f"{template_module_name}.py", content)
        ensure_import(f'addons/{module}/__init__.py', 'models')
        ensure_import(f'addons/{module}/models/__init__.py', template_module_name)

        cleanup_manifest(module)


def convert_records_to_function(all_records, model, function_name, one_level=False):
    ...
    stream = io.StringIO()
    stream.write(
        indent(1, f"def {function_name}(self, template_code):\n")
        + ('tags(' in body and indent(2, "tags = self._get_tag_mapper(template_code)\n") or '')
        + indent(2, "return ")
        + body
    )
...
def cleanup_manifest(module):
    manifest_path = Path.cwd() / f'addons/{module}/__manifest__.py'
    ...
    if 'data' in vals:
        for value in list(vals['data']):
            data_path = Path.cwd() / f'addons/{module}/{value}'
